...
GeoDisplay::GeoDisplay(QWidget* parent) : QMainWindow(parent) {
	...
	setupPanels();			//2.设置选项面板（用 PanelManager）
...
GeoDisplay::~GeoDisplay() {
	...
	// m_optionsPanel 由 PanelManager 挂载的 QDockWidget 接管 parent，
	// 随主窗口销毁自动释放，不在此手动 delete。
	// m_config 由 ConfigManager 单例 owns，不在此 delete
}
...
void GeoDisplay::config_init() {		//配置初始化
	...
	//2.通过 ConfigManager 引导加载配置文件
	m_config = ConfigManager::instance().bootstrap();

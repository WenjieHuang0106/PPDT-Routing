...
//几何变换
void RouterMeshless::geoTransform() {
	m_pushVecLock.clear();
	//1.按网表分组，按网表和端点分组
	unordered_map<string, vector<PathNode*>> netHeads;
	unordered_map<string, unordered_map<Point, vector<PathNode*>, Point::Hash>> netPaths;
	for (const auto& [head, shape] : m_paths) {
		if (shape.edges.empty())
			continue;
		const string& netName = shape.shapeName;
		// 头尾节点
		PathNode* head = shape.edges.front().p1;
		PathNode* tail = shape.edges.back().p2;
		const Point& headPt = head->pos;
		const Point& tailPt = tail->pos;
		// step1: netHeads
		netHeads[netName].emplace_back(head);
		// step2: netPaths
		netPaths[netName][headPt].emplace_back(head);
		if (!(headPt == tailPt)) {
			netPaths[netName][tailPt].emplace_back(tail);
		}
	}
	double stepLen = 5; //初始化，在推挤之前正式赋值
	for (int pushcnt = 0; pushcnt < m_pushTimes; pushcnt++) {
		for (const auto& [netName, heads] : netHeads) {
			m_epsilon = m_netsInfos->at(netName).width * 0.5;
			//2. 对一个net,构建引力场，确定每个net的布线边的推挤方向
			vector<PathNode*> sortedHeads = heads;
			sort(sortedHeads.begin(), sortedHeads.end(),
				[this](PathNode* a, PathNode* b) {
					return m_pathLengths.at(a) > m_pathLengths.at(b);
				});
			const unordered_map<Point, vector<PathNode*>, Point::Hash>& posNodes = netPaths[netName];
			for (PathNode* head : sortedHeads) {
				unordered_map<PathNode*, Point> gravitys;
				buildGravityPath(head, posNodes, gravitys);
				if (gravitys.empty())
					continue;
				//3. 求引力大小
				double maxForce = MapMinValue;
				Point pushDir(0, 0);
				PathNode* pushNode = nullptr;
				for (auto& [node, gravityVec] : gravitys) {
					if (!node->next) break;
					debugBreak(node->pos);
					// 取L_vec的单位法向向量
					Point L_vec = (node->next->pos - node->pos).normalizeVec();	//走线的方向
					Point normal = L_vec.rotate90();
					Point pushVec = normal * (normal * gravityVec);  //normal * pushVec * normal,将引力场投影到法向
					double L_M = (node->next->pos - node->pos).vecLength() * m_gamaL;	//质量
					pushVec = pushVec * L_M; //引力的法向分量
					double F_g_norm = pushVec.vecLength();
					if (F_g_norm > maxForce && node->prev && node->next && node->next->next) {	// 第一条node->prev和最后一条node->next->next不能被推挤
						maxForce = F_g_norm;
						pushDir = pushVec / F_g_norm;	//单位向量
						pushNode = node;
					}
				}
				if (!pushNode) continue;
				//4.记录当前路径的推挤方向，加速收敛
				if (m_pushVecLock.find(head) == m_pushVecLock.end()) {
					m_pushVecLock[head] = pushDir;
				}
				//5. 根据引力方向进行推挤
				if (pushNode) {
					stepLen = calcStepLength(pushNode);
					if (stepLen < MapMinValue)
						break;
					Point pushVec = pushDir * stepLen;
					vector<Point> pushVecs = { pushVec,pushVec };
					bool canPushWithPushVec = pushVecCheck(pushNode, pushVecs, head, posNodes);	//检查推挤向量是否有特殊能与邻居路径节点重合的步长
					if (!canPushWithPushVec)
						continue;
					Point tailPos = m_paths[head].edges.back().p2->pos;
					double maxPathLen = fabs(tailPos.x - head->pos.x) + fabs(tailPos.y - head->pos.y);
					bool pushed = false;
					for (auto& v : pushVecs) {
						if (v.vecLength() < MapMinValue)
							continue;

						//临时推挤
						PolyShape* shape = &m_paths[head];
						PolyShape* shapeTmp = shape->copy();
						bool canPush = checkBeforPush(pushNode, shape, shapeTmp, v, false);
						double pathLen = getOnePathLength(shapeTmp->edges.front().p1);
						shapeTmp->deleteNodes();
						delete shapeTmp;
						shapeTmp = nullptr;
						//如果长度符合，则进行正式推挤
						if (canPush && pathLen < maxPathLen + MapMinValue) {
							pushAndUpdate(pushNode, v, shape);
							pushed = true;
							break;
						}
					}
		...
		//cout << "pushTimes=" << pushcnt << "\t,pushLen" << stepLen << endl;
	}
}
bool RouterMeshless::pushVecCheck(PathNode* pushNode, vector<Point>& pushVecs, PathNode* head, const unordered_map<Point, vector<PathNode*>, Point::Hash>& posNodes) {
	// 对推挤向量进行标准化
	Point& pushVec = pushVecs[0];
	if (pushVec.vecLength() < MapMinValue)
		return false;
	// 只能朝同一个方向推挤，加速收敛
	auto it = m_pushVecLock.find(head);
	if (it != m_pushVecLock.end()) {
		Point& lockVec = it->second;
		if (lockVec * pushVec < 0) {
			lockVec = lockVec - lockVec.normalizeVec() * 0.1;
			return false;
		}
	}
	PathNode* M = pushNode;
	PathNode* N = pushNode->next;
	if (!N)
		return false;
	PathNode* tail = m_paths[head].edges.back().p2;
	const Point& Mpos = M->pos;
	const Point& Npos = N->pos;

	// 1.参考路径检测
	vector<PathNode*> group = posNodes.at(head->pos);
	const auto& group2 = posNodes.at(tail->pos);
	group.insert(group.end(), group2.begin(), group2.end());
	double dmax = 0;
	bool gotPt1OrPtForPush = false;
	for (PathNode* ep : group) {
		if (ep == head || ep == tail)
			continue;
		if (checkLinePtToPush(ep, Mpos, Npos, pushVec)) {	//可推到线段端点
			pushVecs[1] = pushVec;
			break;
		}
	}
	// 2.端点焊盘检测
	double farShadow = (head->pos - Mpos) * pushVec;
	double nearShadow = (tail->pos - Mpos) * pushVec;
	PathNode* A = M->prev;
	while (A && A->pos == Mpos)
		A = A->prev;
	PathNode* B = N->next;
	while (B && B->pos == Npos)
		B = B->next;
	Point maxVec = pushVec, minVec = pushVec;
	if (farShadow < 0 && nearShadow < 0)
		return true;
	Point farPinPos = head->pos;
	Point nearPinPos = tail->pos;
	if (farShadow < nearShadow) {
		swap(farPinPos, nearPinPos);
		swap(farShadow, nearShadow);
	}
	if (checkPadForbidden(farPinPos, Mpos, Npos, pushVecs))
		maxVec = pushVecs[0];
	if (nearShadow > 0) {
		vector<Point> pushVecs2 = { pushVecs[1],pushVecs[1] };
		if (checkPadForbidden(nearPinPos, Mpos, Npos, pushVecs2))
			minVec = pushVecs2[1];
	}
	pushVecs = { maxVec,minVec };
	return true;
}
bool RouterMeshless::checkPadForbidden(const Point& pt, const Point& Mpos, const Point& Npos, vector<Point>& pushVecs) {
	Point& pushVec = pushVecs[0];
	auto it = m_steinerQuery.find(pt);
	if (it == m_steinerQuery.end())
		return false;
	const vector<double>& box = it->second->pin->box;
	double xmin = box[0];
	double ymin = box[1];
	double xmax = box[2];
	double ymax = box[3];
	...
	double dmax = 0;
	double dmin = INF;
	Point dir = pushVec.normalizeVec();
	for (auto& c : corners) {
		double d = (c - Mpos) * dir;
		//double d = c.distanceToLine(Mpos, Npos);
		dmax = max(dmax, d);
		dmin = min(dmin, d);
	}
	double pushVecLen = pushVec.vecLength();
	if (pushVecLen > dmax || pushVecLen < dmin)	//没有进入焊盘区
		return false;
	pushVec = dir * dmax;		//首个元素是最大推挤距离
	pushVecs[1] = dir * dmin;
	return true;
}
bool RouterMeshless::checkLinePtToPush(PathNode* headOrTail, const Point& Mpos, const Point& Npos, Point& pushVec) {
	const Point dir = pushVec.normalizeVec();
	PathNode* cur = headOrTail;
	double dmax = 0;
	bool got = false;
	if (cur->next) {
		while (cur && cur->next) {		//一条路径
			PathNode* neib = cur->next;
			bool inMidOfEdge = Mpos.ptInMidOfEdge(cur->pos, neib->pos, false) || Npos.ptInMidOfEdge(cur->pos, neib->pos, false);
			//bool inMidOfEdge = Mpos.inSameLine(cur->pos, neib->pos) || Npos.inSameLine(cur->pos, neib->pos);
			if (inMidOfEdge) {
				got = true;
				double cur_d = (cur->pos - Mpos) * dir;
				double neib_d = (neib->pos - Mpos) * dir;
				if (cur_d > neib_d) {
					dmax = max(dmax, cur_d);
				}
				else {
					dmax = max(dmax, neib_d);
				}
				break;
			}
			cur = neib;
		...
		while (cur && cur->prev) {		//一条路径
			PathNode* neib = cur->prev;
			bool inMidOfEdge = Mpos.ptInMidOfEdge(cur->pos, neib->pos, false) || Npos.ptInMidOfEdge(cur->pos, neib->pos, false);
			//bool inMidOfEdge = Mpos.inSameLine(cur->pos, neib->pos) || Npos.inSameLine(cur->pos, neib->pos);
			if (inMidOfEdge) {
				got = true;
...
void RouterMeshless::buildGravityPath1(PathNode* head, const unordered_map<Point, vector<PathNode*>, Point::Hash>& posNodes, unordered_map<PathNode*, Point>& gravity) {
	//计算一条路径上所有节点的引力场，并设定锁定线段
	PathNode* tail = m_paths[head].edges.back().p2;
	const Point& headPt = head->pos;
	const Point& tailPt = tail->pos;
	//1.将需要用来产生引力场的路径放入分组中
	vector<PathNode*> group = posNodes.at(headPt);
	const vector<PathNode*>& group1 = posNodes.at(tailPt);
	...
	while (cur && cur != tail) {
		Point gSum(0, 0);
		buildGravityNode1(cur, head, tail, group, gSum);		// 计算当前节点的引力场
		gravity[cur] = gSum;
		cur = cur->next;
	}

}
void RouterMeshless::buildGravityNode1(PathNode* cur, PathNode* head, PathNode* tail, const vector<PathNode*>& group, Point& gSum) {
	int layer = cur->layer;
	if (cur->next->layer != layer)
		return;
	for (PathNode* ep : group) {
		if (ep == head || ep == tail)
			continue;
		...
		if (ecur && ecur->next) {
			Point g;
			while (ecur->next) {
				PathNode* ecur_next = ecur->next;
				if (layer == ecur->layer && layer == ecur_next->layer) {
					clacGravity(ecur->pos, ecur_next->pos, { cur->pos }, m_gamaL, g);
					gSum = gSum + g;
				}
				ecur = ecur->next;
			}
		}
		else if (ecur && ecur->prev) {
			Point g;
			while (ecur->prev) {
				PathNode* ecur_prev = ecur->prev;
				if (layer == ecur->layer && layer == ecur_prev->layer) {
					clacGravity(ecur->pos, ecur_prev->pos, { cur->pos }, m_gamaL, g);
					gSum = gSum + g;
				}
				ecur = ecur->prev;
			}
			...
			cerr << "error in func [buildGravityPos]: cur" << cur->pos << ", head:" << head->pos << endl;
		}
	}
}
void RouterMeshless::buildGravityPath(PathNode* head, const unordered_map<Point, vector<PathNode*>, Point::Hash>& posNodes, unordered_map<PathNode*, Point>& gravity) {
	//计算一条路径上所有节点的引力场，并设定锁定线段
	PathNode* tail = m_paths[head].edges.back().p2;
	const Point& headPt = head->pos;
	const Point& tailPt = tail->pos;
	...
	if (group.size() <= 2)
		return;
	//2.计算这条路径各节点的引力场(需要计算首尾)
	PathNode* cur = head;
	while (cur && cur->next) {
		if (cur->next->layer == cur->layer) {
			Point pos = (cur->pos + cur->next->pos) * 0.5;
			Point gSum(0, 0);
			buildGravityPos(cur->layer, { cur->pos, pos, cur->next->pos }, head, tail, group, gSum);		// 计算当前节点的引力场
			gravity[cur] = gSum;
		}
		cur = cur->next;
	}
}
void RouterMeshless::buildGravityPos(int layer, const vector<Point>& poss, PathNode* head, PathNode* tail, const vector<PathNode*>& group, Point& gSum) {
	//1.计算各路径在该点产生的引力场，存入gs
	vector<Point> gs;
	Point pos_S = head->pos;
	Point pos_E = tail->pos;
	Point p21(pos_S.x, pos_E.y);
	Point p22(pos_E.x, pos_S.y);
	for (PathNode* ep : group) {
		Point gOneSorcePath(0, 0);
		if (ep == head) {
			continue;
			Point g;
			double gamaL = m_gamaL * 0.5;
			clacGravity(pos_S, p21, poss, gamaL, g);
			gOneSorcePath = gOneSorcePath + g;
			clacGravity(pos_S, p22, poss, gamaL, g);
			gOneSorcePath = gOneSorcePath + g;
		}
		else if (ep == tail) {
			continue;
		...
		if (ecur && ecur->next) {
			...
			while (ecur->next) {
				PathNode* ecur_next = ecur->next;
				if (layer == ecur->layer && layer == ecur_next->layer) {
					clacGravity(ecur->pos, ecur_next->pos, poss, m_gamaL, g);
					gOneSorcePath = gOneSorcePath + g;
				}
				ecur = ecur->next;
			}
			...
			while (ecur->prev) {
				PathNode* ecur_prev = ecur->prev;
				if (layer == ecur->layer && layer == ecur_prev->layer) {
					clacGravity(ecur->pos, ecur_prev->pos, poss, m_gamaL, g);
					gOneSorcePath = gOneSorcePath + g;
				}
				ecur = ecur->prev;
			}
			...
			cerr << "error in func [buildGravityPos]: cur" << ", head:" << head->pos << endl;
		}
		gs.emplace_back(gOneSorcePath);
	}
	//2.引力场过滤与合成（取最大）
	 //方案1，取最大值
	//double maxG = 0;
	//for (auto& gOneSorcePath : gs) {
	//	double G_val = gOneSorcePath.vecLength();
	//	if (G_val > maxG) {
	//		maxG = G_val;
	//		gSum = gOneSorcePath;
	//	}
...
double RouterMeshless::calcStepLength(PathNode* node_pre)const {
	if (!node_pre->next)
		return 5;
	double length = node_pre->pos.distanceTo(node_pre->next->pos);
	double stepLength_0 = (1 - m_pushStepdecay) * length / m_1_decay_exp20;
	return stepLength_0;
}

void RouterMeshless::clacGravity(const Point& p1, const Point& p2, const vector<Point>& pgs, const double& gamaL, Point& g)const {
	// 1.质心法，2.分段质心法，3.积分法
	int type = 3;
	switch (type) {
	case 1:
		clacGravity1(p1, p2, pgs, gamaL, g);		// 1.质心法
		break;
	case 2:
		clacGravity2(p1, p2, pgs, gamaL, g);		// 2.分段质心法
		break;
	case 3:
		clacGravity3(p1, p2, pgs, gamaL, g);		// 3.积分法
		break;
	default:
		clacGravity1(p1, p2, pgs, gamaL, g);		// 1.质心法
		break;
...
void RouterMeshless::clacGravity1(const Point& p1, const Point& p2, const vector<Point>& pgs, const double& gamaL, Point& g)const {
	//质心法
	double vx = p2.x - p1.x;
	double vy = p2.y - p1.y;
	double len2 = vx * vx + vy * vy;	//场源线段长度的平方
	if (len2 < MapMinValue) {	// 场源为0或者， || fabs(vx * wy - vy * wx) < MapMinValue
		g.x = 0;
		g.y = 0;
		return;
	}
	...
	for (const Point& pg : pgs) {
		double dx = mx - pg.x;
		double dy = my - pg.y;
		double dist2 = dx * dx + dy * dy;	//质心到点pg的距离
		if (dist2 < MapMinValue) {
			g.x = 0;
			g.y = 0;
		}
		else {
			double coef = gamaL * sqrt(len2) / dist2;
			g.x = dx * coef;
			g.y = dy * coef;
		}
	}
}
void RouterMeshless::clacGravity2(const Point& p1, const Point& p2, const vector<Point>& pgs, const double& gamaL, Point& g)const {
	// 分段质心法，分段累加
	const int N = 5;
	double gDecay = 0.9;
	const double soft = m_epsilon;
	double vx = p2.x - p1.x;
	double vy = p2.y - p1.y;
	double len = sqrt(vx * vx + vy * vy);
	if (len < MapMinValue) { g.x = 0; g.y = 0; return; }
	g.x = 0;
	g.y = 0;
	double dl = len / N;
	for (const Point& pg : pgs) {
		if (N == 1) {
			double px = (p1.x + p2.x) * 0.5;
			double py = (p1.y + p2.y) * 0.5;
			double dx = px - pg.x;
			double dy = py - pg.y;
			double dist2 = dx * dx + dy * dy + soft * soft;
			double coef = gamaL * dl / dist2;
			g.x += dx * coef;
			g.y += dy * coef;
			return;
		}
		for (int i = 0; i < N; i++) {
			...
			double dy = py - pg.y;
			double dist2 = dx * dx + dy * dy + soft * soft;
			int k = i < (N - 1 - i) ? i : (N - 1 - i);
			double weight = pow(gDecay, k);
			double coef = gamaL * dl / dist2 * weight;
			g.x += dx * coef;
			g.y += dy * coef;
		}
	}
	...
	// 分段质心法，分点累加
	/*
	const int N = 3; // 分段数，可调
	const double soft = m_epsilon; //软化距离
	double vx = p2.x - p1.x;
	double vy = p2.y - p1.y;
	double len = sqrt(vx * vx + vy * vy);
	if (len < MapMinValue) { g.x = 0; g.y = 0; return; }
	...
	for (int i = 0; i < N; i++) {
		...
		double dx = px - pg.x;
		double dy = py - pg.y;
		double dist2 = dx * dx + dy * dy + soft * soft; //软化
		double coef = gamaL * dl / dist2;
		g.x += dx * coef;
		g.y += dy * coef;
	}
	...
	//cout << "p1=" << p1 << ", p2=" << p2 << ", pg=" << pg <<"gamaL=" << gamaL << ", g=" << g << endl;
}
void RouterMeshless::clacGravity3(const Point& p1, const Point& p2, const vector<Point>& pgs, const double& gamaL, Point& g)const {
	...
	g.x = 0;
	g.y = 0;
	if (L2 < eps * eps) return;  // 线段长度近似为零
	double L = sqrt(L2);
	double C = L2;
	double sqrt_eps = sqrt(m_epsilon);
	for (const Point& pg : pgs) {
		double a = p1.x - pg.x;
		double b = p1.y - pg.y;
		double A = a * a + b * b;
		...
		if (D < 0) D = 0;
		double r1 = sqrt(A);
		double r2 = sqrt(A + 2 * B + C);
		double u1 = B / C;
		double u2 = 1.0 + u1;
		double sqrt_D_eps = sqrt(D + m_epsilon);

		// 安全计算 atan 参数
		double atan_arg1 = sqrt_eps * u1;
		double atan_arg2 = sqrt_eps * u2;
		if (r1 < 1e-8) {
			if (fabs(atan_arg1) < 1e-12)
				atan_arg1 = 0.0;
			else {
				atan_arg1 = (atan_arg1 > 0) ? PI / 2.0 : -PI / 2.0;
				r1 = 1.0; // 避免除零
			}
		}
		else {
			atan_arg1 = atan_arg1 / r1;
		}
		if (r2 < 1e-8) {
			if (fabs(atan_arg2) < 1e-12) {
				atan_arg2 = 0.0;
			}
			else {
				atan_arg2 = (atan_arg2 > 0) ? PI / 2.0 : -PI / 2.0;
			}
		}
		else {
			atan_arg2 = atan_arg2 / r2;
		}
		double term1 = atan(r2 / sqrt_eps) - atan(r1 / sqrt_eps);
		double term2 = atan(atan_arg2) - atan(atan_arg1);
		double term1_x = (dx / C) * term1;
		double term1_y = (dy / C) * term1;
		double term2_x = (1.0 / sqrt_D_eps) * (a - B * dx / C) * term2;
		double term2_y = (1.0 / sqrt_D_eps) * (b - B * dy / C) * term2;
		g.x += gamaL * (term1_x + term2_x) / sqrt_eps;
		g.y += gamaL * (term1_y + term2_y) / sqrt_eps;
	}
}

void RouterMeshless::detectMergeableEdges(const vector<PathNode*>& nodes, vector<vector<PathNode*>>& gS, vector<vector<PathNode*>>& gE) {
	for (PathNode* node : nodes) {
		...
		for (int d = 0; d < 4; ++d) {
			PathNode* cur = start;
			while (cur) {
				Point diff = cur->pos - node->pos;
				bool stop = false;
				if (d == 0) {
					if (diff.x > 0) {
						gS[0].emplace_back(node);
						gE[0].emplace_back(cur);
						break;
					}
					if (diff.x < 0)
						stop = true;
					...
					if (diff.y > 0) {
						gS[1].emplace_back(node);
						gE[1].emplace_back(cur);
						break;
					}
					if (diff.y < 0)
						stop = true;
					...
					if (diff.x < 0) {
						gS[2].emplace_back(node);
						gE[2].emplace_back(cur);
						break;
					}
					if (diff.x > 0)
						stop = true;
					...
					if (diff.y < 0) {
						gS[3].emplace_back(node);
						gE[3].emplace_back(cur);
						break;
					}
					if (diff.y > 0)
						stop = true;
...
bool RouterMeshless::pushForAtomic(vector<vector<PathNode*>>& gS, vector<vector<PathNode*>>& gE) {

	return true;
}
void RouterMeshless::makeEdgeAtomic(const vector<vector<PathNode*>>& gS, const vector<vector<PathNode*>>& gE) {

}
void RouterMeshless::netPathsToTree(const unordered_map<Point, vector<PathNode*>, Point::Hash>& posNodes) {

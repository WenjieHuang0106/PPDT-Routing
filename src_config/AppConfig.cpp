...
void AppConfig::configUpdate(const QString& qFilePath, const QString& qFileName) {
	m_qFilePath = qFilePath;
	m_qFileName = qFileName;
}

bool AppConfig::read() {
	// 读取路径由 ConfigManager::bootstrap 设置到 m_filePath（config/app_last.config）
	// 文件存在性由 ConfigManager 负责，read() 只负责解析
	...
	while (!in.atEnd()) {
		...
		if (line.startsWith("defaltPath"))
			...
			QMessageBox::warning(nullptr, QStringLiteral("警告："), QStringLiteral("配置文件中存在未知的行！"));
			continue;
		}
	}
	configFile.close();
...
bool AppConfig::save() const {
	// 写入路径用 m_filePath（由 ConfigManager::saveLast 设置为 config/app_last.config）
	// 1. 确保目录存在

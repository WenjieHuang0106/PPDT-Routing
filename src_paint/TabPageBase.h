...
class TabPageBase : public QWidget {
	Q_OBJECT
public:
	explicit TabPageBase(QString qFullName, QString qFilePath, QString qFileName, QWidget* parent = nullptr)
...
private:
	void setupConnections() const {
		connect(m_diagram, &Diagram::leftPressed,
			this, &TabPageBase::leftPressRun);
		connect(m_diagram, &Diagram::leftPressedMove,
			this, &TabPageBase::leftPressMoveRun);
		connect(m_diagram, &Diagram::leftReleaseed,
			this, &TabPageBase::leftReleaseRun);
	}
...
protected:
	virtual void fillData(AppConfig* conf) = 0;
...
public:
	using CreatorFunc = std::function<TabPageBase* (const QString& qFullName, const QString& qFilePath, const QString& qFileName, QWidget* parent)>;
	static TabPageFactory& instance();
	// 注册某个后缀对应的 TabPage 构造函数
	void registerCreator(const QString& suffix, CreatorFunc creator);	
	TabPageBase* create(const QString& qFullName, QWidget* parent);
